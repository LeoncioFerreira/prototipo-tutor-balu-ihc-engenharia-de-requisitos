...
type HomeProps = { onNavigate: (view: "pets" | "community" | "chat") => void; onCompleteMedicine: () => void; onCompleteWalk: () => void; medicineDone: boolean; walkDone: boolean };
...
export function FigmaHomeScreen({ onNavigate, onCompleteMedicine, onCompleteWalk, medicineDone, walkDone }: HomeProps) {
  ...
  return <main className="min-h-[100dvh] bg-[#202124]"><section className="relative mx-auto min-h-[100dvh] w-full max-w-[393px] overflow-hidden"><img src={image} alt="Tela 5: Home do Tutor" className="block h-auto w-full" /><h1 className="sr-only">Olá, Leôncio!</h1><h2 className="sr-only">Nível {medicineDone ? 4 : 3}</h2><input aria-label="Vermífugo Chemital" type="checkbox" checked={medicineDone} onChange={onCompleteMedicine} className="absolute left-[calc(50%+115px)] top-[418px] h-8 w-8 cursor-pointer opacity-0" /><input aria-label="Passeio Diário" type="checkbox" checked={walkDone} onChange={onCompleteWalk} className="absolute left-[calc(50%+115px)] top-[495px] h-8 w-8 cursor-pointer opacity-0" /><button aria-label="Pets" onClick={() => onNavigate("pets")} className="absolute bottom-[21px] left-[23%] h-14 w-[18%]" /><button aria-label="Comunidade" onClick={() => onNavigate("community")} className="absolute bottom-[21px] left-[42%] h-14 w-[24%]" /><button aria-label="Chat" onClick={() => onNavigate("chat")} className="absolute bottom-[21px] right-[5%] h-14 w-[18%]" /></section></main>;
}
