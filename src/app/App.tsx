...
const initialTasks: Task[] = [{ id: "food", time: "08:00", title: "Alimentação", description: "Ração Seca Premier • Feito por Leôncio", done: true, xp: 15 }, { id: "medicine", time: "14:00", title: "Vermífugo Chemital", description: "Dar 1/2 comprimido via oral", done: false, xp: 30 }, { id: "walk", time: "18:00", title: "Passeio Diário", description: "Meta diária: 20 min de caminhada", done: false, xp: 25 }];
const nav = [
...
function Layout({ active, navigate, children }: { active: View; navigate: (view: View) => void; children: React.ReactNode }) {
  return <main className="relative h-[100dvh] overflow-hidden bg-[#202124] text-[#002045]"><div className="mx-auto h-full w-full max-w-[393px] overflow-y-auto bg-[#f7fafc] px-5 pb-[102px] pt-11">{children}</div><nav className="absolute bottom-4 left-1/2 z-20 flex h-[66px] w-[calc(100%-40px)] max-w-[353px] -translate-x-1/2 justify-between rounded-[22px] border border-[#DCE6EF] bg-white px-2 py-2 shadow-[0_8px_16px_rgba(26,54,93,.04)]">{nav.map(({ id, label, asset }) => <button key={id} onClick={() => navigate(id)} aria-label={label} className={`flex h-[50px] w-[70px] flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-medium ${active === id ? "bg-[#E6FFFA] text-[#002045]" : "text-[#6B8297]"}`}><img src={asset} alt="" className="h-[18px] w-[18px]" />{label}</button>)}</nav></main>;
}

function Avatar({ pet, selected = false, large = false }: { pet: typeof pets[number]; selected?: boolean; large?: boolean }) { return <div className={`grid ${large ? "h-14 w-14" : "h-11 w-11"} place-items-center rounded-full border text-lg font-extrabold ${selected ? "border-2 border-[#24b09c] bg-[#e6f7f4]" : "border-[#b2f5ea] bg-[#e6fffa]"}`}>{pet.initial}</div>; }
function Pill({ children }: { children: React.ReactNode }) { return <span className="rounded-full bg-[#e6fffa] px-2.5 py-1 text-[11px] font-semibold text-[#183a78]">{children}</span>; }
function Back({ navigate, to = "home" }: { navigate: (view: View) => void; to?: View }) { return <button onClick={() => navigate(to)} aria-label="Voltar" className="grid h-9 w-9 place-items-center rounded-full bg-white"><ArrowLeft size={20}/></button>; }

function HomeScreen({ navigate, tasks, setTasks }: { navigate: (view: View) => void; tasks: Task[]; setTasks: React.Dispatch<React.SetStateAction<Task[]>> }) {
  const xp = tasks.filter(t => t.done).reduce((sum, t) => sum + t.xp, 55); const level = 3 + Math.floor(xp / 100); const remaining = 100 - (xp % 100);
  const carouselAssets = ["/assets/figma/home/11.svg", "/assets/figma/home/09.svg", "/assets/figma/home/09.svg"];
  return <Layout active="home" navigate={navigate}><header className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#b2f5ea] font-bold">L</span><div><p className="text-[11px] font-medium text-[#4a5568]">Bom dia!</p><h1 className="text-lg font-extrabold">Olá, Leôncio!</h1></div></div><button onClick={() => navigate("chat")} className="grid h-10 w-10 place-items-center rounded-full bg-[#e6f7f4]"><Bell size={19} className="text-[#f8ca5c]"/></button></header><section className="mt-5 flex gap-4">{pets.map((pet,index) => <button key={pet.name} className={`flex flex-col items-center gap-1 text-xs ${index === 0 ? "font-bold" : "text-[#4a5568]"}`}><img src={carouselAssets[index]} alt="" className="h-14 w-14"/>{pet.name}</button>)}<button className="flex flex-col items-center gap-1 text-[10px] font-bold"><img src="/assets/figma/home/06.svg" alt="" className="h-14 w-14"/>Adicionar</button></section><section className="mt-5 flex gap-4 rounded-2xl border border-[#e2e8f0] bg-white p-4 shadow-sm"><img src="/assets/figma/home/xp-level-badge.png" alt="Badge de nível" className="h-12 w-12"/><div className="min-w-0 flex-1"><h2 className="text-sm font-bold">Nível {level}</h2><p className="mt-1 text-[11px] text-[#4a5568]">Faltam {remaining} XP para o Nível {level + 1}</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-[#f2f4f5]"><div className="h-full rounded-full bg-[#24b09c]" style={{ width: `${xp % 100}%` }}/></div></div></section><section className="mt-5"><h2 className="text-base font-extrabold">Rotina de Hoje</h2><div className="mt-3 space-y-3">{tasks.map(task => <label key={task.id} className="flex min-h-[70px] cursor-pointer items-center gap-3 rounded-2xl border border-[#e2e8f0] bg-white p-3.5"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#e6f7f4]"><img src={task.id === "food" ? "/assets/figma/home/08.svg" : task.id === "walk" ? "/assets/figma/home/14.svg" : "/assets/figma/home/08.svg"} alt="" className="h-9 w-9"/></span><span className="min-w-0 flex-1"><b className="block text-[13px]">{task.time} • {task.title}</b><small className="block text-[11px] text-[#4a5568]">{task.description}</small></span><input aria-label={task.title} type="checkbox" checked={task.done} onChange={() => setTasks(current => current.map(item => item.id === task.id ? { ...item, done: !item.done } : item))} className="peer sr-only"/><span className="grid h-6 w-6 place-items-center rounded-lg border-2 border-[#74dfd2] text-white peer-checked:bg-[#24b09c]"><img src="/assets/figma/home/15.svg" alt="" className={task.done ? "h-4 w-4" : "hidden"}/></span></label>)}</div></section></Layout>;
}

function PetsScreen({ navigate }: { navigate: (view: View) => void }) { return <Layout active="pets" navigate={navigate}><h1 className="text-xl font-extrabold">Meus pets</h1><section className="mt-7 h-[170px] rounded-[18px] border border-[#e2e8f0] bg-white py-4 text-center shadow-sm"><div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-[#e6f7f4]"><PawPrint size={38}/></div><h2 className="mt-2 text-lg font-bold">Escolha um pet</h2><div className="mt-2 flex justify-center gap-2"><Pill>3 cadastrados</Pill><Pill>1 principal</Pill></div></section><div className="mt-7 space-y-5">{pets.map(pet => <article key={pet.name} className="min-h-[164px] rounded-[18px] border border-[#c2cad9] bg-white p-3 shadow-sm"><div className="flex gap-3"><Avatar pet={pet}/><div><h2 className="font-semibold text-[#183a78]">{pet.name}</h2><p className="text-[13px] text-[#4a5568]">{pet.details}</p></div></div><div className="mt-3 flex flex-wrap gap-2">{pet.badges.map(b => <Pill key={b}>{b}</Pill>)}</div><div className="mt-4 flex gap-3"><button onClick={() => navigate("pet")} className="rounded-full bg-[#183a78] px-3 py-2 text-[13px] font-semibold text-white">Ver perfil</button><button onClick={() => navigate("appointment")} className="rounded-full border border-[#c2cad9] bg-[#e6fffa] px-3 py-2 text-[13px] font-semibold">Marcar Consulta</button></div></article>)}</div></Layout>; }

function CommunityScreen({ navigate }: { navigate: (view: View) => void }) { return <Layout active="community" navigate={navigate}><h1 className="text-xl font-extrabold">Comunidades</h1><div className="mt-5 flex items-center justify-between"><h2 className="font-semibold">Minhas raças</h2><button className="text-[13px] font-bold text-[#24b09c]">Ver tudo</button></div><div className="mt-4 flex gap-4">{["Caramelo", "Vira-lata", "Gateiros"].map(x => <button key={x} className="flex flex-col items-center gap-1 text-[10px] font-medium"><span className="grid h-[52px] w-[52px] place-items-center rounded-full bg-[#e6f7f4]"><PawPrint size={22}/></span>{x}</button>)}<button className="flex flex-col items-center gap-1 text-[10px]"><span className="grid h-[52px] w-[52px] place-items-center rounded-full border border-[#e2e8f0] bg-white"><Plus/></span>Adicionar</button></div><label className="mt-5 flex h-[42px] items-center gap-2 rounded-full border border-[#e2e8f0] bg-white px-4"><Search size={16}/><input aria-label="Buscar na comunidade" placeholder="Buscar na comunidade..." className="w-full bg-transparent text-sm outline-none"/></label><article className="mt-5 h-[418px] rounded-[18px] border border-[#e2e8f0] bg-white p-4 shadow-sm"><Pill>● Clube dos Caramelos</Pill><p className="mt-3 text-[11px] text-[#4a5568]">2,4 mil membros</p><h2 className="mt-2 max-w-[230px] text-lg font-bold">Uma comunidade para trocar dicas, rotina e cuidado com caramelos</h2><div className="mt-4 rounded-xl border border-[#e2e8f0] p-3"><b className="text-xs">Salomão Rodrigues</b><p className="mt-2 text-[11px]">O Balu soltou muito pelo essa semana e a escova de banho ajudou demais. Quem tem pet com pelagem parecida usa escovação diária?</p><div className="mt-3 flex gap-4 text-[11px] text-[#4a5568]"><button><Heart size={14} className="inline"/> 12 curtidas</button><button><MessageCircle size={14} className="inline"/> 4 comentários</button></div></div><button onClick={() => navigate("club")} className="mt-4 ml-auto block rounded-full bg-[#002045] px-5 py-3 text-[13px] font-bold text-white">Entrar no clube</button></article></Layout>; }

function ChatScreen({ navigate }: { navigate: (view: View) => void }) { const [draft,setDraft] = useState(""); const [messages,setMessages] = useState<{from:"user"|"bot"; text:string}[]>([]); const send=()=>{if(!draft.trim()) return; const text=draft; setMessages(m=>[...m,{from:"user",text},{from:"bot",text:findChatbotReply(text).text}]);setDraft("")}; return <Layout active="chat" navigate={navigate}><header className="flex items-center gap-3"><Back navigate={navigate}/><span className="grid h-9 w-9 place-items-center rounded-full border border-[#b2f5ea] bg-[#e6f7f4]"><PawPrint size={17}/></span><div><h1 className="text-[15px] font-bold">Conversa com Balu</h1><p className="text-[10px] text-[#24b09c]">Online • Assistente Virtual</p></div></header><section className="mt-6 min-h-[45dvh] space-y-3">{messages.length === 0 && <div className="grid place-items-center pt-24 text-center"><img src="/assets/figma/logo-balu.png" alt="Balu" className="h-24 w-24 object-contain"/><p className="mt-3 text-sm text-[#4a5568]">Como posso ajudar o Balu hoje?</p></div>}{messages.map((m,i) => <p key={i} className={`max-w-[82%] rounded-2xl px-4 py-3 text-[12px] ${m.from === "bot" ? "bg-[#e6f7f4]" : "ml-auto border border-[#e2e8f0] bg-white"}`}>{m.text}</p>)}</section><aside className="rounded-xl border border-[#fed7d7] bg-[#fff5f5] p-3 text-[10px] text-[#c53030]"><CircleHelp size={14} className="mr-2 inline"/>A assistente virtual não substitui uma consulta. Se os sintomas persistirem, agende um atendimento.</aside><div className="mt-3 flex gap-2 overflow-x-auto"><button className="whitespace-nowrap rounded-full bg-[#c53030] px-3 py-2 text-[11px] font-bold text-white">Acionar Emergência</button><button className="whitespace-nowrap rounded-full border px-3 py-2 text-[11px]">Dicas de Saúde</button></div><div className="mt-3 flex items-center rounded-2xl border border-[#e2e8f0] bg-white p-2"><input aria-label="Mensagem" value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>e.key === "Enter" && send()} placeholder="Escreva sua mensagem..." className="h-10 flex-1 px-2 text-sm outline-none"/><button aria-label="Enviar" onClick={send} className="grid h-9 w-9 place-items-center rounded-full bg-[#b2f5ea]"><Send size={16}/></button></div></Layout>; }
