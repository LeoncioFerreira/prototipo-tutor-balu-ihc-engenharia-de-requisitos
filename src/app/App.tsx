...
function ExactHomeScreen({ navigate }: { navigate: (view: View) => void }) {
  const source = "/assets/figma/tela-05-home.png";
  return <FigmaCanvas source={source} alt="Tela inicial do tutor Balu" navigate={navigate} />;
}
...
function FrameHotspots({ navigate }: { navigate: (view: View) => void }) {
  return <div className="absolute inset-x-[5.1%] bottom-[2.35%] h-[7.75%]" aria-label="Navegação inferior">{([['home','Início','left-0 w-[19.8%]'],['pets','Pets','left-[22.5%] w-[19.8%]'],['community','Comunidade','left-[45.2%] w-[26.6%]'],['chat','Chat','right-0 w-[19.8%]']] as const).map(([view,label,position]) => <button key={view} aria-label={label} onClick={() => navigate(view)} className={`absolute inset-y-0 ${position}`} />)}</div>;
}

function FigmaCanvas({ source, alt, navigate }: { source: string; alt: string; navigate: (view: View) => void }) {
  return <main className="grid h-[100dvh] w-screen place-items-center overflow-hidden bg-[#202020]"><div className="relative aspect-[393/852] h-[100dvh] max-w-full"><img src={source} alt={alt} className="block h-full w-full" /><FrameHotspots navigate={navigate} /></div></main>;
}

function ExactFrame({ view, navigate }: { view: Exclude<View, "login" | "home" | "chat">; navigate: (view: View) => void }) {
  const source = frameByView[view];
  return <FigmaCanvas source={source} alt={`Tela ${view} do Balu`} navigate={navigate} />;
}

function ChatScreen({ navigate }: { navigate: (view: View) => void }) { const [draft,setDraft]=useState(""); const [reply,setReply]=useState(""); const send=()=>{ if(!draft.trim())return; setReply(findChatbotReply(draft).text); setDraft("");}; return <main className="grid h-[100dvh] w-screen place-items-center overflow-hidden bg-[#202020]"><div className="relative aspect-[393/852] h-[100dvh] max-w-full"><h1 className="sr-only">Converse com Balu</h1><img src="/assets/figma/tela-14.png" alt="Tela de conversa com Balu" className="block h-full w-full" /><div aria-hidden="true" className="absolute left-[5.1%] top-[11.3%] h-[53.3%] w-[89.8%] bg-[#f7fafc]" /><button aria-label="Voltar ao início" onClick={() => navigate("home")} className="absolute left-[5.1%] top-[5.1%] h-[4.3%] w-[6%]" /><div className="absolute left-[5.1%] top-[79.9%] h-[9.6%] w-[89.8%]"><input aria-label="Mensagem" value={draft} onChange={(event)=>setDraft(event.target.value)} onKeyDown={(event)=>event.key==="Enter"&&send()} className="absolute left-[5%] top-[6%] h-[40%] w-[76%] bg-transparent text-[clamp(8px,3.5vw,14px)] text-[#183a78] outline-none" /><button aria-label="Enviar" onClick={send} className="absolute right-0 top-[8%] h-[42%] w-[11%]" /></div>{reply && <p aria-live="polite" className="absolute left-[7%] top-[14%] max-w-[68%] rounded-[12px] bg-white p-[3%] text-[clamp(8px,3.1vw,12px)] text-[#4a5568] shadow-sm">{reply}</p>}<FrameHotspots navigate={navigate} /></div></main>; }
